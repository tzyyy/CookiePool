...
class CookiePool:
    ...
    def save(self, cookie_id, s_cookies, n_alive_time=3600):
        ...
        try:
            ...
            # n_result = self.__update(dic_data)    # 0.1.5版本后优化mysql操作,故注释，但是保留逻辑流程
            n_result = 0
            if 0 == n_result:
    ...
    def __insert(self, dic_data):
        ...
        try:
            ...
            dic_update = dic_data.copy()
            del dic_update["md5"]
            s_update = ", ".join(' {} = VALUES({})'.format(k,k) for k in dic_update.keys())
            n_sql = "insert into {}(%s) values(%s) ON DUPLICATE KEY UPDATE {};".format(self._s_table_name, s_update)
